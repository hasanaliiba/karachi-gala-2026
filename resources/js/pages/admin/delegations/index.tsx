...
function VerifyActions({ delegationId }: { delegationId: number }) {
        ...
        <div className="flex flex-wrap items-center gap-2">
            <Button
                ...
                onClick={() => {
                    ...
                    post(`/admin/delegations/${delegationId}/verify`);
                }}
            >
                Approve
            </Button>
...
export default function AdminDelegationsIndex({ delegations }: { delegations: Delegation[] }) {
    return (
        <>
            ...
            <div className="space-y-4">
                ...
                {delegations.map((delegation) => (
                    <div key={delegation.id} className="space-y-3 rounded-md border border-cyan-400/20 bg-[#13123A] p-4">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    {delegation.user.name} ({delegation.user.email}) | {delegation.type} | {delegation.status}
                                </p>
                            </div>
                            <div className="flex items-center gap-3">
                                <p className="text-sm font-semibold">Amount Due: PKR {delegation.payment?.amount_due ?? delegation.grand_total_fee}</p>
                                <Link
                        ...
                        <VerifyActions delegationId={delegation.id} />
                        {delegation.payment?.rejection_reason && (
                            <p className="text-xs text-red-300">Last rejection: {delegation.payment.rejection_reason}</p>
                        )}
                    </div>
                ))}
            </div>
